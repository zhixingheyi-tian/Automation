...
unset SPARK_HOME
$SPARK_PHIVE_HOME/sbin/stop-thriftserver.sh
...
if [ $1 -ne 2 ]; then
  sh ${tpcds_script_home}/run_stream.sh 1 ${dbname_file} | tee -a $LOG 2>&1 &
  sh ${tpcds_script_home}/run_stream.sh 2 ${dbname_file} | tee -a $LOG 2>&1 &
  sh ${tpcds_script_home}/run_stream.sh 3 ${dbname_file} | tee -a $LOG 2>&1 &
  sh ${tpcds_script_home}/run_stream.sh 4 ${dbname_file} | tee -a $LOG 2>&1 &
else
  sh ${tpcds_script_home}/run_stream.sh 5 ${dbname_file} | tee -a $LOG 2>&1 &
  sh ${tpcds_script_home}/run_stream.sh 6 ${dbname_file} | tee -a $LOG 2>&1 &
  sh ${tpcds_script_home}/run_stream.sh 7 ${dbname_file} | tee -a $LOG 2>&1 &
  sh ${tpcds_script_home}/run_stream.sh 8 ${dbname_file} | tee -a $LOG 2>&1 &
fi
