...
val queries = queries_list.map { l =>
   spark.read.parquet(s"$root_dir/result/q${l}_result").createOrReplaceTempView(s"result_${l}")
   val queryContent: String = Source.fromFile(s"${log_location}/tpcds-queries/q${l}.sql").mkString
   val df = spark.sql(s"$queryContent")
   df.createOrReplaceTempView(s"df_${l}")
   val diff = spark.sql(s"select * from df_${l} EXCEPT select * from result_${l}")
   if(diff.head(1).isEmpty) {
        out.println(s"Q${l} passes.")
    } else {
	    out.println(s"Q${l} fails.")
    }
}
out.close
