...
for round in $(seq $iteration);do
    ...
    for t in ${queries[@]};do
        ...
        if [ -e "${queriesdir}/q${t}.sql" ]; then
            if [ $t == 75 ]; then
                if [ -n "${arrow_enable}" ] && [ "${arrow_enable}" = "arrow" ] ;then
                    $SPARK_PHIVE_HOME/bin/beeline -u jdbc:hive2://{%hostname%}:10000 -i ${dbname_file} -f ${queriesdir}/q${t}_native.sql > ${log_current_dir}/q${t}.log 2>&1
                else
                    $SPARK_PHIVE_HOME/bin/beeline -u jdbc:hive2://{%hostname%}:10000 -i ${dbname_file} -f ${queriesdir}/q${t}.sql > ${log_current_dir}/q${t}.log 2>&1
                fi
            else
                $SPARK_PHIVE_HOME/bin/beeline -u jdbc:hive2://{%hostname%}:10000 -i ${dbname_file} -f ${queriesdir}/q${t}.sql > ${log_current_dir}/q${t}.log 2>&1
            fi
        ...
        if [ -e "${queriesdir}/q${t}a.sql" ]; then
            $SPARK_PHIVE_HOME/bin/beeline -u jdbc:hive2://{%hostname%}:10000 -i ${dbname_file} -f ${queriesdir}/q${t}a.sql > ${log_current_dir}/q${t}a.log 2>&1
            wait $! #$!表示上个子进程的进程号
        ...
        if [ -e "${queriesdir}/q${t}b.sql" ]; then
            $SPARK_PHIVE_HOME/bin/beeline -u jdbc:hive2://{%hostname%}:10000 -i ${dbname_file} -f ${queriesdir}/q${t}b.sql > ${log_current_dir}/q${t}b.log 2>&1
            wait $! #$!表示上个子进程的进程号
