...
start_time=$(date +%s)
${SPARK_PHIVE_HOME}/bin/beeline --showStartEndTime=true -u jdbc:hive2://{%hostname%}:10000 -i $2 -f ${queries_dir}/queries/query_$1.sql > ${LOG_DIR}/stream_$1.out 2> ${LOG_DIR}/stream_$1.err
end_time=$(date +%s)
