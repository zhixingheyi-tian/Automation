...
def gen_data(custom_conf):
    ...
    master = get_master_node(slaves)
    beaver_env = get_merged_env(custom_conf)
    gen_tpc_data(master, custom_conf, beaver_env, "tpch")
...
def run_query(custom_conf, iteration):
    ...
    status = run_tpc_query(master, slaves, beaver_env, iteration, "tpch")
...
def run_query_with_validation(custom_conf, runType):
    ...
    master = get_master_node(slaves)
    beaver_env = get_merged_env(custom_conf)
    run_tpc_query_validation(master, slaves, beaver_env, "tpch", runType)

def deploy(custom_conf, plugins):
    print(colors.LIGHT_BLUE + "Deploy TPC-H" + colors.ENDC)
    cluster_file = get_cluster_file(custom_conf)
    slaves = get_slaves(cluster_file)
    master = get_master_node(slaves)
    beaver_env = get_merged_env(custom_conf)
    ...
    deploy_tpch_dbgen(custom_conf, master, slaves)
