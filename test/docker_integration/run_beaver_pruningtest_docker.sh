...
expect {
        "~]#" {send "cp -r /home/$project_name/conf/ /opt/conf1;echo \"$master_hostname $master_ip root bdpe123 master\" > /opt/conf1/nodes.conf;echo \"$slave1_hostname $slave1_ip root bdpe123 slave\" >> /opt/conf1/nodes.conf;sed -i 's/power_test_0=1-30/power_test_0=1-30/g' /opt/conf1/BB/conf/bigBench.properties\r"}
}
expect {
...
expect {
        "~]#" {send "rm -rf /etc/yum.repos.d/CentOS-*;cd /home/$project_name/;test/pruningtest/deployHoS.py /home/$project_name/test/pruningtest /opt/conf1/\r"}
}
expect {
