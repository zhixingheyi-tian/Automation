...
expect {
        ...
        "~]#" {send "echo 'http_proxy=http://child-prc.intel.com:913' >> ~/.bashrc;echo 'ftp_proxy=http://child-prc.intel.com:913' >> ~/.bashrc;echo 'https_proxy=https://child-prc.intel.com:913' >> ~/.bashrc;echo 'export http_proxy' >> ~/.bashrc; echo 'export ftp_proxy' >> ~/.bashrc; echo 'export https_proxy' >> ~/.bashrc;source ~/.bashrc;source /home/$project_name/bin/setup-env.sh\r"}
}
expect {
        "~]#" {send "cp -r /home/$project_name/conf/  /opt/sqlconf;echo \"$master_hostname $master_ip root bdpe123 master\" > /opt/sqlconf/nodes.conf;echo \"$slave1_hostname $slave1_ip root bdpe123 slave\" >> /opt/sqlconf/nodes.conf;sed -i 's/SPARK_VERSION=2.0.0/SPARK_VERSION=2.0.0-hive/g' /opt/sqlconf/env.conf\r"}
}
expect {
...
expect {
        "~]#" {send "rm -rf /etc/yum.repos.d/CentOS-*;cd /home/$project_name/;python benchmark/TPCDSonSparkSQL.py deploy_and_run /opt/sqlconf/\r"}
}
expect {
        "]#" {send "cd /home/$project_name/test/;source test_utils.sh;service_check;hadoop fs -get /spark/sql/performance/* /opt/Beaver/result/;exit\r"}
}
expect eof
EOF
