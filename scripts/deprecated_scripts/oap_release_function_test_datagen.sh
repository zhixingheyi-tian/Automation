   ...
   do
       if [ -d $repo/$list ] ; then
           ...
           if [[ $list =~ "oap_cache" ]]; then
               ...
               sh $BEAVER_HOME/scripts/daily_test.sh gen_data $repo/$list $plugin
           #oap-native-sql and oap-data-source
               ...
               sh $BEAVER_HOME/scripts/tpc_h.sh run_validation $repo/$list baseline
           # oap-shuffle
           elif [[ $list =~ "oap_shuffle_TPCDS" ]]; then
               sh $BEAVER_HOME/scripts/spark.sh update $repo/$list
               sh $BEAVER_HOME/scripts/spark.sh gen_data $repo/$list
               sh $BEAVER_HOME/scripts/spark.sh run_validation $repo/$list baseline
           #oap-spark
